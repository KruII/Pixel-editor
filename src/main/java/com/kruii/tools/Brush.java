...
public class Brush implements Tool {
    ...
    public Brush() {
        ...
        JTextField sizeField = new JTextField(String.valueOf(brushSize), 3);
        sizeField.addKeyListener(new java.awt.event.KeyAdapter() {
            @Override
            public void keyReleased(java.awt.event.KeyEvent e) {
                try {
                    int val = Integer.parseInt(sizeField.getText());
                    brushSize = Math.max(1, val);
                } catch (NumberFormatException ex) { 
                    brushSize=1; 
                }
                sizeField.setText(String.valueOf(brushSize));
            }
        });
    ...
    public void onMouseMove(MouseEvent e, PixelCanvas canvas, PixelModel model) {
        new ColorBorder(e, canvas, Color.BLUE, brushSize);
    }
